...
const LAYERS = (D: AnalysisResult) => [
  { icon: '🚇', name: '교통 접근성', score: D.scores.traffic,     detail: D.traffic_detail,     key: 'traffic' },
  { icon: '🏪', name: '생활 인프라', score: D.scores.infra,       detail: D.infra_detail,       key: 'infra' },
  { icon: '📚', name: '학군 환경',   score: D.scores.school,      detail: D.school_detail,      key: 'school' },
  { icon: '🔊', name: '소음·환경',   score: D.scores.noise,       detail: D.noise_detail,       key: 'noise' },
  { icon: '🛍️', name: '상권 활성도', score: D.scores.commerce,    detail: D.commerce_detail,    key: 'commerce' },
  { icon: '🏗️', name: '개발 잠재력', score: D.scores.development, detail: D.development_detail, key: 'development' },
];

const STEPS = ['교통 데이터 수집 중...', '생활 시설 분석 중...', '소음 데이터 연동 중...', 'AI 종합 평가 생성 중...'];

const sc = (s: number) => s >= 80 ? 'var(--main)' : s >= 60 ? '#7a8a5e' : '#aaa';
    ...
    <div className={styles.page}>
      ...
      <div className={styles.searchBox}>
        ...
        <div className={styles.quickChips}>
          ...
          {['마포구 성산동', '강남구 역삼동', '용산구 이태원동', '송파구 잠실동'].map(a => (
        ...
        {recentSearches.length > 0 && (
          <div className={styles.recentRow}>
            ...
            <button className={styles.btnClear} onClick={() => { setRecentSearches([]); localStorage.removeItem('moveiq_history'); }}>지우기</button>
          </div>
        )}
      </div>
        ...
        <div className={styles.resultBox}>
          ...
          <div className={styles.tabs}>
            {[
              ...
              <button key={t} className={`${styles.tab} ${rTab === t ? styles.tabActive : ''}`} onClick={() => setRTab(t)}>
                {label}
              </button>
            ))}
          </div>
                ...
                <div className={styles.schoolList}>
                  {D.school_info.map((s, i) => (
                    <div key={i} className={styles.schoolItem}>
                      ...
                      <div style={{ textAlign: 'right' }}><span className={styles.schoolRating}>{s.rating}</span><span className={styles.schoolNote}>{s.note}</span></div>
                    </div>
                  ))}
                </div>
              ) : (
              ...
              {D.reviews?.length ? (
                D.reviews.map((r, i) => (
                  <div key={i} className={styles.reviewItem}>
                    <div className={styles.reviewHeader}><span className={styles.reviewAuthor}>{r.author}</span><span className={styles.reviewStars}>{'★'.repeat(r.rating)}{'☆'.repeat(5 - r.rating)}</span><span className={styles.reviewDate}>{r.date}</span></div>
                    <p>{r.text}</p>
                  </div>
                ))
              ) : (
              ...
              {D.jeonse_risk ? (
                <>
                  <div className={`${styles.jeonseRiskBadge} ${styles[`jeonseRisk_${D.jeonse_risk.level}`]}`}>
                    {D.jeonse_risk.level === 'high' ? '🔴 전세사기 위험 지역' : D.jeonse_risk.level === 'medium' ? '🟡 주의 필요' : '🟢 비교적 안전'}
                  </div>
      ...
      <header className={styles.header}>
        <div className={styles.headerInner}>
          <Link href="/" className={styles.logo}>
            <div className={styles.logoMark}/><span>무브IQ</span>
          </Link>
